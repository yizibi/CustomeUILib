...
//    self.title = self.title?self.title:[NSString stringWithFormat:@"%ld/%ld",self.currentPhotoIndex+1,self.dataSource.count];
    self.title = self.title?self.title:self.photoName;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell.singleTapGestureBlock) {
        cell.singleTapGestureBlock = ^(){
            if (weakSelf.isHideNaviBar==YES) {
                [weakSelf setIsHideNaviBar:NO];
            }
            else{
                [weakSelf setIsHideNaviBar:YES];
    ...
    LXPositionView *viewA = [[LXPositionView alloc] initWithFrame:CGRectMake(0, 0, 20, 20) animationType:AnimationTypeWithBackground];
    viewA.center = CGPointMake(cell.imageView.width*(self.device_pos_x)+5, cell.imageView.height*(self.device_pos_y)+5);
    viewA.multiple = 1.5;
    [cell.imageView addSubview:viewA];
